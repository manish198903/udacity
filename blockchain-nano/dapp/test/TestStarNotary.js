...
it('can add the star name and star symbol properly', async() => {
    ...
    let instance = await StarNotary.deployed();
    assert.equal(await instance.name.call(), 'Mthakrani Star Notary Token')
    assert.equal(await instance.symbol.call(), "MSNT");
});
...
it('lets a user transfer a star', async() => {
    ...
    let instance = await StarNotary.deployed();
    let tokenId = 8;    
    await instance.createStar('Star 8!', tokenId, {from: accounts[0]});
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    assert.equal(await instance.lookUptokenIdToStarInfo.call(tokenId), "Star 9!");
});